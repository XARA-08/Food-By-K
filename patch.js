(function(){
  ...
  function removeDashboard(){
    var d=document.getElementById('page-dashboard'); if(d)d.remove();
  }
  ...
  // CRITICAL: override openItem to work with Supabase UUID keys
  function patchOpenItem(){
    ...
    window.openItem=function(id){
      ...
      if(si){
        if(item.img){si.src=item.img;si.style.display='block';}
  ...
  function init(){
    ...
    removeDashboard();
    patchLogoTap();
    patchOpenItem(); // CRITICAL: patch openItem before loadMenu
    loadBanner();
    loadMenu();
    loadEvents();
    loadHours();
