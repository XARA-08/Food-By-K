...
(function () {
  ...
  function patchOpenItem() {
    ...
    window.openItem = function (id) {
      ...
      /* ── CRITICAL FIX ──────────────────────────────────────
         confirmUpsell() and skipUpsell() in index.html do:
           const item = ITEMS[curItem];
           addToBasket(item.name, item.price, curQty);
         
         curItem is now a UUID. ITEMS has slug keys only.
         ITEMS[uuid] = undefined → addToBasket crashes silently
         → no basket entry, no toast, no order possible.
         
         Fix: inject the UUID key into the live ITEMS object
         so ITEMS[uuid] resolves correctly for ALL downstream
         functions (confirmUpsell, skipUpsell, addToBasket).
      ─────────────────────────────────────────────────────── */
      try {
        if (window.ITEMS) {
          window.ITEMS[id] = { name: item.name, desc: item.desc, price: item.price, img: item.img };
        }
      } catch(e) {}
