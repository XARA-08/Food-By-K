/* ═══════════════════════════════════════════════════════════════
   Food By K — patch.js  (SECURED BUILD v3)
   Loads menu/events/hours/banner from Supabase.
   Does NOT override confirmCollection — that lives in index.html.
   ═══════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  var sb;
  /* Expose sb on window so index.html confirmCollection can use it */
  window._fbkSb = null;
  var _fbkItems = {};
    ...
    try {
      ...
      r.data.forEach(function(ev){
        ...
        card.innerHTML=
          ...
          (ev.is_recurring?'background:#2A2218;color:#D9C9B0;border:1px solid rgba(242,100,25,0.18);':'background:#D94F00;color:white;')+'">'+(ev.is_recurring?'🔁 Recurring':'⚡ Special Event')+'</span>'+
  ...
  /* ── Init ────────────────────────────────────────────────── */
  function init() {
    try { sb=supabase.createClient(SB_URL,SB_KEY); window._fbkSb=sb; } catch(e){ console.warn('[FBK] Supabase init failed'); return; }
    patchLogoTap();
    patchOpenItem();
    loadBanner();
    loadMenu();
