...
(function () {
  ...
  async function loadMenu() {
    ...
    try {
      ...
      // Store in our own dict
      window._FBK = window._FBK || {};
      r.data.forEach(function (item) {
        window._FBK[item.id] = { name: item.name, desc: item.description || '', price: item.price, img: item.image_url || '' };
      });
      // Also inject into the original ITEMS const so openItem/chQty work natively
      try {
        if (typeof ITEMS !== 'undefined') {
          r.data.forEach(function (item) {
            ITEMS[item.id] = { name: item.name, desc: item.description || '', price: item.price, img: item.image_url || '' };
          });
        }
      } catch(e) {}
      // Fallback: override openItem to use our dict if ITEMS injection failed
      if (typeof window._fbkOpenItemPatched === 'undefined') {
        window._fbkOpenItemPatched = true;
        var _origOpen = window.openItem;
        window.openItem = function(id) {
          var fi = window._FBK && window._FBK[id];
          if (fi) {
            window.curItem = id; window.curQty = 1;
            var si = document.getElementById('sheet-img');
            if (fi.img) { si.src = fi.img; si.style.display = 'block'; var sw = document.querySelector('.sheet-img-wrap'); if(sw) sw.style.height = '48vw'; }
            else { si.style.display = 'none'; var sw2 = document.querySelector('.sheet-img-wrap'); if(sw2) sw2.style.height = '0'; }
            document.getElementById('sheet-name').textContent = fi.name;
            document.getElementById('sheet-desc').textContent = fi.desc;
            document.getElementById('sheet-price').textContent = 'R' + fi.price + '.00';
            var qt = document.getElementById('qty-tot'); if(qt) qt.textContent = 'R' + fi.price + '.00';
          ...
          } else if (typeof _origOpen === 'function') { _origOpen(id); }
        };
      }
      var grouped = {}, order = [];
      r.data.forEach(function (item) {
